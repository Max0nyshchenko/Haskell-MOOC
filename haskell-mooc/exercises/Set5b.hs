...
cull val (Node a l r) = if a == val then Empty else Node a (cull val l) (cull val r)
...
isOrdered (Node a l r) = cmp a l (>) && cmp a r (<) && isOrdered l && isOrdered r
    where cmp x Empty op = True
          cmp x (Node v _ _) op = op x v 
...
search :: (Eq a, Ord a) => a -> Tree a -> Maybe [Step]
search s Empty = Nothing
search s (Node a l r)
  | s == a = Just []
  | otherwise  = case (search s l, search s r) of 
    (Just pathL, _) -> Just (StepL : pathL)
    (_, Just pathR) -> Just (StepR : pathR)
    _ -> Nothing
