...
frequenciesProp freq (NonEmpty input) = let output = freq input in outputInInput input output .&&. sumIsLength input output .&&. inputInOutput input output
...
genList = do
  len <- choose (3, 5)
  fmap sort $ vectorOf len (elements [0 .. 10])
