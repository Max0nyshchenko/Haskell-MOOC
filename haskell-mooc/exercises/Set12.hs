...
instance Functor TwoList where
  ...
  fmap f (TwoNode l r list) = TwoNode (f l) (f r) (fmap f list)
...
count v = foldr (\x acc -> if v == x then acc + 1 else acc) 0
...
inBoth listUno = foldr (\x acc -> if elem x listUno then x : acc else acc) []
...
instance Foldable TwoList where
  ...
  foldr f acc (TwoNode l r rest) = f l $ f r $ foldr f acc rest
