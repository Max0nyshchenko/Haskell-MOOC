...
danger a [] = False
danger a (x : xs) = (sameRow a x || sameCol a x || sameDiag a x || sameAntidiag a x) || danger a xs
...
fixFirst n ((r, c) : sx)
  | c > n = Nothing
  | danger (r, c) sx = fixFirst n (nextCol (r, c) : sx)
  | otherwise = Just ((r, c) : sx)
...
backtrack (c : s : stack) = nextCol s : stack
