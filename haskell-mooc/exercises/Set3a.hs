...
capitalize = unwords . map captal . words 
  where captal (x:xs) = toUpper x : xs
        captal _ = ""
...
joinToLength n list = [ z | x <- list, y <- list, let z = x ++ y, length z == n]
...
interpreter commands = interpret commands 0 0 []
  where interpret [] _ _ output = reverse output
        interpret (command : rest) x y output = case command of
          "up" -> interpret rest x (y + 1) output
          "down" -> interpret rest x (y - 1) output
          "left" -> interpret rest (x - 1) y output
          "right" -> interpret rest (x + 1) y output
          "printY" -> interpret rest x y (show y : output)
          "printX" -> interpret rest x y (show x : output)
          _ -> interpret rest x y output
