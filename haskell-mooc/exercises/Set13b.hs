...
mapM2 op xs ys = mapM (\(x, y) -> op x y) $ zip xs ys
...
visit maze place = do
  state <- get
  unless (elem place state) $ do
    modify (\state' -> place : state)
    case lookup place maze of
      Nothing -> return ()
      Just neighbours -> forM_ neighbours (\neighbour -> visit maze neighbour)
...
path maze place1 place2 = elem place2 $ execState (visit maze place1) []
...
allSums xs = map sum $ combine xs
  where
    combine [] = [[]]
    combine (y : ys) = combine ys ++ map (y :) (combine ys)
...
instance Monad Result where
  ...
  return a = MkResult a
  (>>=) (MkResult a) fn = fn a
  (>>=) NoResult fn = NoResult
  (>>=) (Failure err) fn = Failure err
...
instance Functor SL where
  -- implement fmap
  fmap f (SL sl) = SL (\int -> let (a, int', log) = sl int in (f a, int', log))
...
instance Monad SL where
  -- implement return and >>=
  return a = SL (\int -> (a, int, []))
  (>>=) (SL sl) fn =
    SL
      ( \int ->
          let (a, int', msgs) = sl int
              SL sl' = fn a
              (b, int'', msgs') = sl' int'
           in (b, int'', msgs ++ msgs')
      )
