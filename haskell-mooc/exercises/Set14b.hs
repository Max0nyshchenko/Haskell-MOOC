...
deposit db accountName moneyAmount = execute db depositQuery (accountName, moneyAmount)
...
balance db accountName = do
  res <- query db balanceQuery [accountName] :: IO [[Int]]
  return $ sum $ concat res

withdraw :: Connection -> T.Text -> Int -> IO ()
withdraw db accountName moneyAmount = deposit db accountName (- moneyAmount)
...
parseCommand (cmd : acc : xs)
  | cmd == T.pack "balance" = if null xs then Just (Balance acc) else Nothing
  | cmd == T.pack "withdraw" = parseInt (T.concat xs) >>= \int -> Just (Withdraw acc int)
  | otherwise = parseInt (T.concat xs) >>= \int -> Just (Deposit acc int)
parseCommand xs = Nothing
...
perform db cmd = case cmd of
  Just (Balance acc) -> balance db acc >>= \int -> pure $ T.pack $ show int
  Just (Deposit acc int) -> deposit db acc int >> pure (T.pack "OK")
  Just (Withdraw acc int) -> withdraw db acc int >> pure (T.pack "OK")
  Nothing -> pure (T.pack "ERROR")
...
server db request respond = do
  ...
  text <- perform db $ parseCommand path
  putStrLn $ "text response is : " ++ T.unpack text
  respond (responseLBS status200 [] $ encodeResponse text)
