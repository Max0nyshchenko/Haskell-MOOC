...
winner scores player1 player2 =
  lookup player1 scores >>= (\score1 -> lookup player2 scores >>= (\score2 -> compare score1 score2))
  where
    compare s1 s2
      | s1 == s2 || s1 > s2 = Just player1
...
selectSum :: (Num a, Ord a) => [a] -> [Int] -> Maybe a
...
countAndLog f xs = let list = xs >>= (\x -> if f x then [show x] else []) in Logger list (length list)
