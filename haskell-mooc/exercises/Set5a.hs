...
up p = MkPosition {x = x p, y = y p + 1}

-- right increases the x value of a position by one
right :: Position -> Position
right p = MkPosition {x = x p + 1, y = y p}
...
fromBin :: Bin -> Int
fromBin bin = turn bin 0
  where turn End pow = 0
        turn (O b) pow = 0 + turn b (pow+1)
        turn (I b) pow = (2^pow) + turn b (pow+1)

toBin :: Int -> Bin
toBin n = foldr (\x acc -> inc acc) (O End) [1..n]
