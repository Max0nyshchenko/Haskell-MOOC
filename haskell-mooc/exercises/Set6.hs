...
instance Eq a => Eq (List a) where
  (LNode x listX)==(LNode y listY) = x==y && listX==listY
  Empty == Empty = True
  _ == _  = False
...
instance Num RationalNumber where
  ...
  signum (RationalNumber a b) = simplify $ RationalNumber (signum a) (signum b)
...
class Cycle a where
  step :: a -> a
  stepMany :: Int -> a -> a
  stepMany 0 item = item
  stepMany n item = stepMany (n-1) (step item)
