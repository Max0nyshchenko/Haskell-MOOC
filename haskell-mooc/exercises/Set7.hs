...
member el (Set xs) = elem el xs

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add el (Set xs) = Set $ sort $ nub $ el:xs
...
data State = Start | EggsAdded | FlourAdded | SugarAdded | FlourSugarAdded | Mixed | Error | Finished
  deriving (Eq,Show)
...
reverseNonEmpty (def:|[]) = def:|[]
reverseNonEmpty (def:|list) = last list:|reverse (def:(init list))
...
passwordAllowed str (ContainsSome n) = any (\c -> elem c n) str
passwordAllowed str (DoesNotContain n) = not (passwordAllowed str (ContainsSome n))
...
data Arithmetic = Literal Integer | Mult Arithmetic Arithmetic | Add Arithmetic Arithmetic
  deriving Show
