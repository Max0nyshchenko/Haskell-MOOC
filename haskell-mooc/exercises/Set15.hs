...
statements xs ys = liftA2 (++) xs (liftA2 (++) [" is ", " is not "] ys)
...
calculator "negate" n = negate <$> readMaybe n
calculator "double" n = (* 2) <$> readMaybe n
...
validateDiv n d = check (d /= 0) "Division by zero!" (div n d)
...
validateAddress streetName streetNumber postCode = Address <$> checkStreet <*> checkNumber <*> checkCode
  where
    checkStreet = check (length streetName < 21) "Invalid street name" streetName
    checkNumber = check (all isDigit streetNumber) "Invalid street number" streetNumber
...
twoPersons ::
  ...
  f Bool ->
  f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = (\x y -> [x, y]) <$> (Person <$> name1 <*> age1 <*> employed1) <*> (Person <$> name2 <*> age2 <*> employed2)
...
boolOrInt s = (Left <$> pBool (readMaybe s :: Maybe Bool)) <|> (Right <$> pInt (readMaybe s :: Maybe Int))
  where
    pInt (Just n) = pure n
    pInt _ = invalid "Not an Int"
    pBool (Just n) = pure n
    pBool _ = invalid "Not a Bool"
...
normalizePhone str = check (length strippedStr <= 10) "Too long" strippedStr *> checkDigits
  where
    strippedStr = filter (not . isSpace) str
    checkDigits = traverse (\x -> check (isDigit x) ("Invalid character: " ++ [x]) x) strippedStr
...
parseExpression str = checkExpression $ words str
  where
    operatorCheck x = check (length x == 1 && elem x ["-", "+"]) ("Unknown operator: " ++ x) (buildOperator x)
    buildOperator "-" = Minus
    buildOperator _ = Plus
    checkNumber x = check (all isDigit x) ("Invalid number: " ++ x) (Number $ read x)
    checkVariable x = check (length x == 1 && (isAlpha $ head x)) ("Invalid variable: " ++ x) (Variable $ head x)
    checkArg x = checkNumber x <|> checkVariable x
    checkExpression [arg1, op, arg2] = operatorCheck op <*> checkArg arg1 <*> checkArg arg2
    checkExpression _ = invalid ("Invalid expression: " ++ str)
...
instance Functor Priced where
  fmap f (Priced int a) = Priced int (f a)

instance Applicative Priced where
  pure a = Priced 0 a
  liftA2 f (Priced intA a) (Priced intB b) = Priced (intA + intB) (f a b)
...
tryAll f (l : list) = f l <|> tryAll f list
tryAll f [] = empty
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap func (Both f) = Both ((fmap . fmap) func f)
