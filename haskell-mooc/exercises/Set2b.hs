...
myGcd a 0 = a
myGcd 0 b = b
myGcd a b = if a > b then myGcd newNum b else myGcd a newNum
  where newNum = abs (a - b)
...
countdown n = "Ready! " ++ (numsDown n "") ++ " Liftoff!"
  where numsDown 1 str = str ++ "1... "
        numsDown i str = numsDown (i-1) (str ++ (show i ++ "... "))
...
smallestDivisor n = dividerFunc n 2
  where dividerFunc n k = if mod n k == 0 then k else dividerFunc n (k+1)
...
isPrime n = case n of 0 -> False
                      1 -> False
                      n -> smallestDivisor n == n
