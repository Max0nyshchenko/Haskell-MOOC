...
blendColor (Color a b c) (Color x y z) = Color (avg a x) (avg b y) (avg c z)
  where
    avg uno dos = div (uno + dos) 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f (Picture a) (Picture b) = Picture (\x -> f (a x) (b x))
...
union (Shape f1) (Shape f2) = Shape fn
  where
    fn coord = f1 coord || f2 coord

cut :: Shape -> Shape -> Shape
cut (Shape f1) (Shape f2) = Shape fn
  where
    fn coord = f1 coord && not (f2 coord)
...
paint (Picture p) (Shape s) (Picture p2) = Picture f
  where
    f coord
      | s coord = p coord
      | otherwise = p2 coord
...
instance Transform Zoom where
  apply (Zoom n) (Picture p) = Picture (p . zoomCoord n)
...
instance Transform Flip where
  apply FlipX (Picture p) = Picture f
    where
      f (Coord x y) = p (Coord (- x) y)
  apply FlipY (Picture p) = Picture f
    where
      f (Coord x y) = p (Coord x (- y))
  apply FlipXY (Picture p) = Picture f
    where
      f (Coord x y) = p (Coord y x)
...
instance Transform BlurMany where
  apply (BlurMany n) (Picture p) = foldr (\x acc -> apply Blur acc) (Picture p) [1 .. n]
